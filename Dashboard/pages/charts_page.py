...
def pagina_graficos(posts: pd.DataFrame):
    ...
    if all(col in posts.columns for col in ["street", "avenue", "number"]):
        posts["endereco_completo"] = (
            ...
            + ", "
            + posts["number"].astype(str)
            + " - "
            + posts["avenue"].astype(str)
        )
    else:
    ...
    if "createdBy" in posts.columns:
        df_user_counts = (
            posts.groupby("createdBy")["id"]
            .count()
            .reset_index()
            .rename(columns={"id": "qtd_posts"})
        )
        fig_user_counts = px.bar(
            ...
            color="createdBy",
            text="qtd_posts",
            title="Número de posts por usuário",
        )
    ...
    # ==========================
    # 💸 Preço médio por usuário
    # ==========================
    if {"price", "createdBy"}.issubset(posts.columns):
        st.divider()
        df_user_price = (
            posts.groupby("createdBy")["price"]
            .mean()
            .reset_index()
            .rename(columns={"price": "preco_medio"})
        )
        fig_user_price = px.bar(
            df_user_price,
            x="createdBy",
            y="preco_medio",
            color="createdBy",
            text="preco_medio",
            title="Preço médio por usuário",
        )
        fig_user_price.update_traces(
